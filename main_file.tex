...
\\
We can also compare the performance while fixing the data size as in Table \ref{tab:level_effect40} and Table \ref{tab:level_effect4800}, which represent the case data is insufficient and the case data is abundant, respectively. One may find the PPL is particular high at target service level 0.63. This can be explained by the difference in overstock/understock costs in profit functions. For the three given functions, the one with target service level 0.63 happens to have highest overstock/understock costs.
...
\footnote{See appendix for figures that not included here.}

\subsection{Nonlinear cost functions} \label{sub:exp2}

In this subsection, we consider the scenario where the error term of the demand follows normal distribution, and the model of the data generating process is known, but the underlying profit function is nonlinear:
\[
    \pi(Q,y)=
    \begin{cases}
    ...
    \end{cases}
\]
where $u\sim \mathcal{N}(30,5^2)$. One can use technique mentioned in \cite{KK18} or other numerical approaches to find the target service level of this function to be 0.56.

In the experiment, both the percentage profit loss and the service level will be concerned. The data will still be generated from ARIMA $(1,0,0)(1,0,0)_4$ process, and we use the performance of DGP to compare with both methods. 
...
We illustrate and compare the proposed method with conventional disjoint method in computational experiments. As expected, the proposed method can achieve as same level of performance as disjoint method in classic NVPs, while generates marvelous results when the scenario become complicated. Specifically, in the case we possess misleading information, our proposed method performs well, while the disjoint method converge to the wrong target. Besides, the proposed method behaves well in the nonlinear profit function case where the quantile regression is no longer functional. It takes less learning data for the proposed method to converge to target than for the disjoint method. The results of experiment also backup our proof on the transformation property between proposed method and quantile regression in the linear profit function case.
