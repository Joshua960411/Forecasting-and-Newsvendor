...
$\tau$ as a ``target service level", since that aiming for this target will bring the company maximised expected profit.
...
For the purpose of what follows, we now explain one variant, the `Nonlinear Newsvendor Problem' (NNV), in detail (see also \cite{BT06,HOS12,HN16,KC62,Kh95,KK18,Mi59,PSC15,PD99,Po90,Po02}). In the NNV, the profit function takes the form:
\[
    \pi(Q,y)=
    \begin{cases}
...
After estimating the model, we can set our next order quantity to $\hat{Q}_{s+1}$. We remark that, while we present the linear regression model here, dynamic models, such as ARIMA or ETS \cite{Hy08}, could be used instead just as efficiently.
...
Another interesting feature of IMEO is that it reduces to quantile regression in the case of the classical NVP (see Appendix \ref{app:C}). Therefore, again in the classical case, our method
...
Figure \ref{fig:fr0.3} represents the average FR. As one might expect, the average fill rate gets better as sample size grows in all methods. Only thing we should mention here is that the DJ requires much larger sample size than other integrated methods to achieve same FR.
...
We would like to stress that, unlike disjoint method, IMEO does not need any complicated numerical optimisation or simulation methods to estimate the optimal order quantity. In addition, we have found that, for these instances, it typically required less computing time than the disjoint method. For example, only $\nicefrac{1}{10}$ amount of computing time is needed by using our proposed method comparing with DJ when $s=40$. Besides, the IMEO results presented above are generated by limited-memory BFGS, which cannot guarantee global optimum. One can apply other algorithms so as to balance between profit achieved and computational time. 
...
Now, we explore the case in which underlying profit function is nonlinear, as stated in Subsection \ref{sub:exp2}. Again, three scenarios are considered.
...
Figure \ref{fig:misnon_under} represents the scenario in which our applied model is \emph{too simple}, merely AR(1) process. Particularly, we can see the PPLs are rather similar in Figure \ref{fig:misnon_under}(a) and Figure \ref{fig:mis_under}(a), and the SL of all three methods converge to our target very fast. In Figure \ref{fig:misnon_over}, on the other hand, we can see the performance of methods in the scenario in which our applied model is \emph{too complicated}, seasonal ARIMA$(1,0,0)(1,0,0)_4$ process. The performances are very similar to the ones we presented in Figure \ref{fig:mis_over}. However, we can see the DJ method performs poorly with nonlinear profit function, regarding to SL, when sample size is small, while IMEO is not influenced by this change.
