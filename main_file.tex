...
\includegraphics{linear-norm-plot_files/figure-latex/}
\label{fig:sl0.3}
\end{figure}

Table \ref{tab:size_effect0.3} shows the full results for a target of 0.3.
As one might expect, the average fill rate follows a similar pattern to
the service level. Moreover, the fill rate is very high for all methods
and all data sizes.\footnote{{\bf It might be better to present the fill rate
in the same way that we presented the average loss and service level. At the moment, it looks a bit strange.}}\\
