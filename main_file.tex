...
Initially, we chose the following parameters for our (linear) profit function:
\begin{itemize}
    \item $p=20$, $v=10$, $c_h=-3$, $c_s=-7$.
\end{itemize}
One can check that the corresponding pair $\big( c_u,c_o \big)$ is $(3,7)$, and the target service level evaluates to $0.3$. (We explore the effect of varying
...
\end{enumerate}
 ...
 Figure \ref{fig:ppl0.3} shows the average PPL obtained when the target service level was set to $0.3$. As one would hope, the losses for all methods converge as the data size grows. It is also apparent that PM and QR have very similar performance. This is to be expected, given that PM is equivalent to QR in the linear case.
...
We see that all four methods converge to the desired target of 0.3 as the data length grows. Interestingly, DJ approaches the target from below, while PM and QR approach it from above. A possible explanation is as follows. In DJ, the estimated variance tends to be larger than the true variance when the data length is small. This causes the order quantity often to be less than it should be. On the other hand, the integrated methods take the underage and overage costs into account. For the given cost parameters, over-stocking is less costly than under-stocking.

Figure \ref{fig:fr0.3} represents the average FR. As one might expect, the average fill rate follows a similar pattern to the service level. Moreover, the fill rate is very high for all methods and all data sizes. The full results for a target of 0.3 can be found in Table \ref{tab:size_effect0.3}.
...
Next, we explored whether the target service level has a significant effect on the performance of the methods. Specifically, we considered the following two alternative parameter settings:
\begin{itemize}
...
and $s=4800$, respectively. Generally speaking, the picture is similar, regardless of the target service level. An exception is that the PPL is particular high with a target service level of 0.63. This is probably due, however, merely to the relatively large overage and underage costs in that case.
...
\subsection{Nonlinear cost functions} \label{sub:exp2}

In this subsection, we examine the relative performance of the methods when applied to the nonlinear newsvendor problem. As before, we assume that the DGP for the demands is an ARIMA $(1,0,0)(1,0,0)_4$ process, and the error term follows a normal distribution. We use the following nonlinear profit function:
\[
    \pi(Q,y)=
    \begin{cases}
    ...
    \end{cases}
\]
where $u\sim \mathcal{N}(30,5^2)$. One can use the technique mentioned in \cite{KK18}, or other numerical approaches, to verify that the target service level of this function is 0.56.
...
Figure \ref{fig:non} shows the PPL and SL of each method, for each data size. It is apparent that both methods perform rather well in terms of PPL, with very similar values for all data sizes. The picture is very different with SL. Indeed, we were surprised to find that the SL of the proposed method is very close to the the target service level even when the data size is small. By contrast, the disjoint method needs more data to reach the target service level, and it approaches it from below.
...
We would like to stress that, even for the NNVP, the proposed method doesn't need any complicated numerical optimisation or simulation methods to estimate the optimal order quantity. Indeed, we found that, for these instances, it typically required less computing time than the disjoint method. (For brevity,
we do not present detailed computing times.)
...
In more detail, the data is still generated from a seasonal ARIMA $(1,0,0)(1,0,0)_4$ model, but we ignore seasonality when solving the NVP. In other words, when estimating the optimal order quantity, we use the incorrect assumption that the DGP is merely an AR$(1)$ process. As in Subsection \ref{sub:exp1}, we assume that the error term of the demand follows a normal distribution, and we use the linear profit function with target service level
$=0.3$.

The percentage profit losses for this case can be seen in Figure
\ref{fig:mis} (a). By comparing with Figure \ref{fig:ppl0.3}, we see that the use of an incorrect model causes both DJ and PM to incur a small additional loss in profit. This loss does not significantly improve as the number of data points increases, presumably because seasonality plays a key role when making ``one-step ahead" forecasts. Interestingly, the performance of DJ and PM is similar, even though they are very different approaches.
...
Next, we consider the opposite situation, in which our model of the DGP is too
complicated. The data is still generated from a seasonal ARIMA $(1,0,0)(1,0,0)_4$ model, but we use a $(2,0,0)(1,0,0)_4$ model when solving the NVP. The results for this case can be seen in Figure \ref{fig:mis} (b). Both DJ and PM incur a loss in profit, as before. Now, however, the loss decreases as the data length increases. Indeed, closer inspection of the regression output revealed that, in both DJ and PM, the estimated value of the redundant model parameter tended to zero as the data length increased.

Figure \ref{fig:mis} (b) also reveals that, for this case, our method is slightly worse than the disjoint method for all data lengths. We do not have a convincing explanation for this phenomenon.

Finally, we explore the effect on DJ and PM when the \emph{error term} (or ``noise") is misspecified. In particular, we generate data using a modified version of our seasonal ARIMA $(1,0,0)(1,0,0)_4$ model, in which the error term follows a Laplace distribution with mean 0 and standard deviation 200. (We remark that the Laplace distribution has ``fat tails", or, more formally, much higher kurtosis than the normal distribution.) When estimating the optimal order quantity, however, we use the incorrect assumption that the error term follows a normal distribution with the same mean and variance.

The results for this scenario are presented in Figure \ref{fig:err}.
By comparing Figure \ref{fig:err}(a) with Figure \ref{fig:ppl0.3}, we see that the incorrect model of the noise leads only to a very small loss in profit.
When we compare Figure \ref{fig:err}(b) with Figure \ref{fig:sl0.3}, the picture is rather different. As before, PM rapidly converges to the target service level (0.3) from above. DJ, on the other hand, gives a rather poor service level, and this drawback is not remedied by an increase in the data length. A possible explanation is that our integrated approach works directly with the data, and is therefore, in a sense, ``non-parametric". This makes it more robust than the disjoint approach to deviations from normality.
...
We illustrate and compare the performance of proposed method with conventional disjoint method in computational experiments. As expected, the proposed method can achieve as same level of performance as disjoint method in classic NVPs, while generates marvelous results when the scenario becomes complicated. Specifically, in the case we possess misleading information, our proposed method still performs well, while the disjoint method converge to the wrong target. Besides, the proposed method generates stable results in the nonlinear profit function case where the quantile regression is no longer functional. Moreover, it takes less learning data for the proposed method to converge to target than for the disjoint method. The results of experiment also backup our proof on the transformation property between proposed method and quantile regression in the linear profit function case.

For future research, it is interesting to consider using a nonlinear form of regression model to estimate the order quantity. Besides, this paper only considers the retailing of single product. In future study, multi-product case can be considered, along with the substitution effect and more.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\printbibliography
