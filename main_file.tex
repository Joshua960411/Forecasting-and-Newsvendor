...
By fixing the target service level, we can compare the performance of methods in different data size in Table \ref{tab:size_effect0.3}. The value in columns of percentage profit loss and average service level are in line with Figure \ref{fig:ppl0.3} and Figure \ref{fig:sl0.3}. The columns of fill rate, on the other hand, provides us more information about demand fulfilment. The service level represents the proportion of iterations that the demand is successfully meet, while the fill rate provides not only the demand is meet or not, but in what percentage the demand was filled. In that case, we can see that the proposed method maintains a very high fill rate in all data size. 
