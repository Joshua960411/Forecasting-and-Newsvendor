...
The results are presented in tables and figures. In Figure \ref{fig:ppl0.5}, average percentage profit loss are calculated based on iterations. Particularly, this figure represents the average performance of methods regarding to percentage profit loss in the situation where the target service level $=0.5$. DGP denotes the exact ARIMA model from data generating process as out upper bound, DJ denotes the disjoint method, QR denotes the quantile regression method, and CF denotes our proposed method. We can see both the proposed method and the benchmark methods converge to the upper bound when data length grows. Besides, the performance of our proposed method and quantile regression is very similar. This is exactly what we expected base on the transformation property we proved in Section \ref{se:new}. Regarding the percentage profit loss, when the data size is small, the disjoint method performed slightly better than the quantile regression method than the proposed method. In Figure \ref{fig:sl0.5}, every points represent the proportion of iterations that the demand is satisfied (e.g. 0.5 means the demand are satisfied in 10,000 of the 20,000 iterations). We can see that the disjoint method performed poorly on the service level when data size is small.

More details about the size effect can be seen in Table \ref{tab:size_effect0.5}.

\begin{figure}[h]
\centering
\includegraphics{ppl0.5-1.pdf}
\caption{Percentage profit loss vs. data size at 0.5 target service level}
\label{fig:ppl0.5}
\end{figure}

\begin{figure}[h]
\centering
\includegraphics{sl-1.pdf}
\caption{Service level vs. data size at 0.5 target service level}
\label{fig:sl0.5}
\end{figure}

\begin{table}[h]
\caption{Size effect on percentage profit loss and service level at 0.5 target service level}
\label{tab:size_effect0.5}
