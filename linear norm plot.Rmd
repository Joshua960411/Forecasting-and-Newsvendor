...
plot.new()
l <- legend(0, 0, bty='n',c("DGP", "DJ","QR","CF"),plot=FALSE, pch=1:4,col=1:4)
w <- grconvertX(l$rect$w, to='ndc') - grconvertX(0, to='ndc')
par(omd=c(0, 1-w, 0, 1))
matplot(mac, type = c("b"),pch=1:4,col = 1:4,xaxt = "n"
        ,xlab = 'data length',ylab = 'percetage profit loss') 
axis(1, at=1:5, labels=x_axis)
legend(par('usr')[2], par('usr')[4], bty='n', xpd=NA
       ,c("DGP", "DJ","QR","CF"), pch=1:4,col=1:4)
...
axis(1, at=1:5, labels=x_axis)
legend(par('usr')[2], par('usr')[4], bty='n', xpd=NA
       ,c("Upper-bound", "DJ","QR","CF"), pch=1:4,col=1:4)
...
```{r size0.5}
dgp_p<-c(Reduce("+",re_40_p_l[[1]])[2]/iter,Reduce("+",re_120_p_l[[1]])[2]/iter
         ,Reduce("+",re_480_p_l[[1]])[2]/iter,Reduce("+",re_1200_p_l[[1]])[2]/iter
         ,Reduce("+",re_4800_p_l[[1]])[2]/iter)
dj_p<-c(Reduce("+",re_40_p_l[[2]])[2]/iter,Reduce("+",re_120_p_l[[2]])[2]/iter
         ,Reduce("+",re_480_p_l[[2]])[2]/iter,Reduce("+",re_1200_p_l[[2]])[2]/iter
         ,Reduce("+",re_4800_p_l[[2]])[2]/iter)
qr_p<-c(Reduce("+",re_40_p_l[[3]])[2]/iter,Reduce("+",re_120_p_l[[3]])[2]/iter
         ,Reduce("+",re_480_p_l[[3]])[2]/iter,Reduce("+",re_1200_p_l[[3]])[2]/iter
         ,Reduce("+",re_4800_p_l[[3]])[2]/iter)
cf_p<-c(Reduce("+",re_40_p_l[[4]])[2]/iter,Reduce("+",re_120_p_l[[4]])[2]/iter
         ,Reduce("+",re_480_p_l[[4]])[2]/iter,Reduce("+",re_1200_p_l[[4]])[2]/iter
         ,Reduce("+",re_4800_p_l[[4]])[2]/iter)
dgp_s<-c(Reduce("+",re_40_p_l[[1]])[3]/iter,Reduce("+",re_120_p_l[[1]])[3]/iter
         ,Reduce("+",re_480_p_l[[1]])[3]/iter,Reduce("+",re_1200_p_l[[1]])[3]/iter
         ,Reduce("+",re_4800_p_l[[1]])[3]/iter)
dj_s<-c(Reduce("+",re_40_p_l[[2]])[3]/iter,Reduce("+",re_120_p_l[[2]])[3]/iter
         ,Reduce("+",re_480_p_l[[2]])[3]/iter,Reduce("+",re_1200_p_l[[2]])[3]/iter
         ,Reduce("+",re_4800_p_l[[2]])[3]/iter)
qr_s<-c(Reduce("+",re_40_p_l[[3]])[3]/iter,Reduce("+",re_120_p_l[[3]])[3]/iter
         ,Reduce("+",re_480_p_l[[3]])[3]/iter,Reduce("+",re_1200_p_l[[3]])[3]/iter
         ,Reduce("+",re_4800_p_l[[3]])[3]/iter)
cf_s<-c(Reduce("+",re_40_p_l[[4]])[3]/iter,Reduce("+",re_120_p_l[[4]])[3]/iter
         ,Reduce("+",re_480_p_l[[4]])[3]/iter,Reduce("+",re_1200_p_l[[4]])[3]/iter
         ,Reduce("+",re_4800_p_l[[4]])[3]/iter)
df<-data.frame(c('40','120','480','1200','4800'),
               percent(dgp_p,accuracy = 0.01),percent(dj_p,accuracy = 0.01)
               ,percent(qr_p,accuracy = 0.01),percent(cf_p,accuracy = 0.01)
               ,percent(dgp_s,accuracy = 0.01),percent(dj_s,accuracy = 0.01)
               ,percent(qr_s,accuracy = 0.01),percent(cf_s,accuracy = 0.01))
colnames(df)<-c('Data size','DGP','disjoint','quantile','proposed','DGP','disjoint','quantile','proposed')
kable(df,'latex', booktabs = T,caption = 'Size effect (q=50%)',align = 'c') %>% 
        add_header_above(c(" ", "Percentage profit loss" = 4, "Average service level" = 4),bold = T) %>%
        kable_styling(latex_options = c("striped", "scale_down"))
```
...
df<-data.frame(c('0.5','0.63','0.3'),percent(dgp_p,accuracy = 0.01),percent(dj_p,accuracy = 0.01)
               ,percent(qr_p,accuracy = 0.01),percent(cf_p,accuracy = 0.01)
               ,percent(dgp_s,accuracy = 0.01),percent(dj_s,accuracy = 0.01)
               ,percent(qr_s,accuracy = 0.01),percent(cf_s,accuracy = 0.01))
